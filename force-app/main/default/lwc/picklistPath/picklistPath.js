...
export default class PicklistPath extends LightningElement {
    ...
    processItems(items){
        if(this.selectedItem){
            ...
            if(selectedItem && selectedItem.length){
                ...
                items.forEach(item => {
                    if(!found){
                        if(item.label === this.selectedItem || item.value === this.selectedItem){
                            found = true;
    ...
    hasSelectedItem(items){
        return items.filter(item => (item.label === this.selectedItem || item.value === this.selectedItem));
    }
    ...
    pathClickHandler(event){
        const clickedItem = {value : event.target.closest('li').dataset.value, label : event.target.closest('li').dataset.label};
    ...
    pathCompleteHandler(){
        if(this.picklistItems && this.picklistItems.length){
            ...
            const pathComplete = new CustomEvent('pathcomplete', {detail:lastItem});
