...
export default class SprintListItem extends LightningElement {
    ...
    sprintClickHandler(){
        const sprintSelect = new CustomEvent('sprintselect', {detail:this.sprintId});
        this.dispatchEvent(sprintSelect);

        fireEvent(this.pageRef, 'sprintselect', this.sprintId);
    }
    ...
    get releaseIcon(){
        if(this.status){
            switch(this.status){
                case 'On Track' : return 'utility:date_time';
                case 'Completed' : return 'utility:success';
                case 'Blocked' : return 'utility:error';
                default : return 'utility:date_time';
            }
        }
